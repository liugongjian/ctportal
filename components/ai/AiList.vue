  ...
  <div>
    ...
    <div  class="pic-bg">
      <div class="section">
        ...
        <div class="list-wrapper-single-line">
          <list-item v-for="item in bodyList" :item="item" :key="item.src"/>
        </div>
      </div>
    </div>
    <div  class="section light-bg">
      ...
      <div class="list-wrapper-single-line">
        <list-item v-for="item in senarioList" :item="item" :key="item.src"/>
      </div>
    </div>
  </div>
</template>
...
export default class extends Vue{
  ...
  private bodyList = [
      { src:'gathering', title:'人员聚集', content:'适用于3米以上的中远距离俯拍，以头部为识别目标统计图片中的瞬时人数' },
      ...
      { src:'helmet', title:'安全帽反光服检测', content:'检测画面中的人员是否佩戴安全帽、穿反光服，规范从业人员的作业行为。' },
  ]
  private senarioList = [
      { src:'dangerzone', title:'危险区域检测', content:'对危险监控区域内的人员靠近行为进行识别。' },
      { src:'fire', title:'烟雾明火检测', content:'对画面中的烟雾、明火进行识别，适用于楼宇、工厂、加油站等场景。' },
      { src:'plate', title:'车牌识别', content:'自动识别车辆车牌信息，应用于停车场、小区、工厂等场景，实现无卡、无人的车辆进出场自动化、规范化管理。' }
  ]
}
</script>
